...
// DRAWING_CANVAS_SIZE is defined once in this module via calculate::mod re-export
...
pub fn drawing_process_genetic(
    ...
    loop {
        ...
        for _ in 0..swaps_per_generation {
            ...
            let bx = (ax as i16 + rng.gen_range(-(max_dist_a as i16)..(max_dist_a as i16 + 1))).clamp(0, settings.sidelen as i16 - 1) as u16;
            let by = (ay as i16 + rng.gen_range(-(max_dist_a as i16)..(max_dist_a as i16 + 1))).clamp(0, settings.sidelen as i16 - 1) as u16;
            ...
            if (bx as i32 - ax as i32).abs() > max_dist_b as i32 || (by as i32 - ay as i32).abs() > max_dist_b as i32 {
                continue;
            }
...
// Minimal stub of drawing_process to provide constants referenced by cli branch.
// This does NOT implement the full drawing genetic algorithm; it's a compatibility shim
// so that the conservative integration (init_canvas) can compile and run.

// The GUI/main uses 128 for DRAWING_CANVAS_SIZE; mirror that here.
pub const DRAWING_CANVAS_SIZE: usize = 128;

// A placeholder for SWAPS_PER_GENERATION_PER_PIXEL if referenced elsewhere.
